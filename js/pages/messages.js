...
class MarkAllAsReadAction {
	constructor() {
		this.getUnreadCountUrl = `https://privatemessages.pekora.zip/v1/messages/unread/count`
		this.getMessagesUrl = `https://privatemessages.pekora.zip/v1/messages?pageSize=20&messageTab=Inbox&pageNumber=`
		this.markAsReadUrl = `https://privatemessages.pekora.zip/v1/messages/mark-read`
...
pageInit.messages = function() {
	document.$watch(">body", body => body.classList.add("btr-messages")).$then()
		.$watch(".messagesContainer-0-2-31").$then()
			.$watchAll(".vTab-0-2-35", container => {
				const inbox = container.$find("#MessagesInbox")
				if(inbox) {
					inbox.$watchAll(".Pekora-message-row", row => {
				...
				if(content) {
					content.$watch(">.Pekora-message-body", body => {
	...
	modifyTemplate("messages-nav", template => {
		...
		const markAsUnread = template.$find(".Pekora-markAsUnreadInbox")
		if(markAsUnread) {
			markAsUnread.after(
				" ",
