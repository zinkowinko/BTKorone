	...
	if(settings.profile.embedInventoryEnabled && window.top !== window) {
		document.$watch("head", head => head.append(html`<base target="_top"></base>`))
			...
			.$watchAll("script", script => {
				if(script.innerHTML.includes("Pekora.DeveloperConsoleWarning.showWarning()")) {
					script.remove()
				}
			})
		...
		document
			...
			.$on("click", ".btr-it-remove", () => {
				...
				function removeItem(index) {
					...
					const url = `https://api.pekora.zip/Marketplace/ProductInfo?assetId=${item.assetId}`
					$.fetch(url).then(async response => {
						const data = await response.json()
						if(validAssetTypes.indexOf(data.AssetTypeId) === -1) { return console.log("Bad assetType", data) }

						$.fetch("https://www.pekora.zip/asset/delete-from-inventory", {
							method: "POST",
							credentials: "include",
