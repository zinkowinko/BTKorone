...
function CreateNewVersionHistory(assetId, assetType) {
	...
	async function loadPage(page) {
		...
		for(let i = innerPageStart; i <= innerPageEnd; i++) {
			const url = `https://www.pekora.zip/places/version-history?assetID=${assetId}&page=${i}`
		...
		script.innerHTML = "Pekora.BootstrapWidgets.SetupPopover()"
	...
	document.documentElement
		.$on("click", ".btr-version-revert", e => {
			...
			$.fetch("https://www.pekora.zip/places/revert", {
				method: "POST",
				credentials: "include",
		...
		.$on("click", ".btr-version-download", e => {
			...
			const assetUrl = `https://assetdelivery.pekora.zip/v1/asset/?id=${assetId}&version=${version}`
...
pageInit.placeconfig = function(placeId) {
	...
	document.$on("click", ".btr-downloadAsZip:not(.disabled)", e => {
		...
		const loadFile = () => {
			...
			const tryDownload = () => {
				const url = `https://assetdelivery.pekora.zip/v1/asset/?id=${placeId}&version=${version}`
		...
		const url = `https://api.pekora.zip/assets/${placeId}/versions`
