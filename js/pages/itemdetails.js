...
;(async () => {
  try {
    const response = await chrome.runtime.sendMessage({
      method: "fetch", // This matches the listener name in your background code
      args: [
        "https://www.koromons.xyz/api/items", 
        { method: "GET" }
      ]
    });
	//console.log(response)
    if (response && response.success) {
      // The background code you shared returns a 'dataUrl' (Base64)
      // We need to decode that back into JSON
      const base64Data = response.dataUrl.split(',')[1];
      const jsonString = atob(base64Data);
      const values = JSON.parse(jsonString);

      //console.log("Successfully fetched via background:", values);
    } else {
      //console.log("failed")
	}
  } catch (error) {
...
const initPreview = async (assetId, assetTypeId, isBundle) => {
	...
	if(settings.itemdetails.itemPreviewer && (isPackage || isBundle || isPreviewable)) {
		...
		if(isBundle) {
			const url = `https://catalog.pekora.zip/v1/bundles/${assetId}/details`
...
const btn = html`
<div>
    <a class="btr-explorer-button" data-toggle="popover" data-bind="btr-explorer-content" style="margin-right: 20px;margin-top: -12px;">
        <span class="btr-icon-explorer"></span>
    </a>
    <div class="rbx-popover-content" data-toggle="btr-explorer-content">
        <div class="btr-explorer-parent" style="z-index: 255;"></div>
	</div>
</div>`


	//alert("watching for noshadow!")

	document.$watch('[class*="itemContainer"]', p => {
		//console.log("found noshadow!")
	
		p.$watch('[class*="itemDetailsContainer"]', c => {
			//console.log("found item details container!")
			c.$watch('[class*="itemHeaderContainer"]', g => { 
				//console.log("found item header!")
				g.$watch('[class*="dropdownContainer"]', j => {
					j.children[0].style.order = 1;
					//console.log("found dropdown!")
					j.append(btn)
					g.classList.add("btr-explorer-btn-shown")
				})
			})
		})
	})

	const btn2 = document.querySelector('.btr-explorer-button')
const popover = document.querySelector('.rbx-popover-content')
	popover.hidden = true
btn2.onclick = e => {
	e.stopPropagation()
	...
	btn.$watchAll(".rbx-popover-content", popover => {
		...
		if(!explorerInitialized) {
			...
			if(assetTypeId === 32) { // Package, I disabled package exploring elsewhere
				//console.log("type is 32.")
				AssetCache.loadText(assetId, text => text.split(";").forEach(id => {
					AssetCache.loadModel(id, model => explorer.addModel(id.toString(), model))
				}))
			} else {
				//console.log("not 32, loading model!", assetId)
				AssetCache.loadModel(assetId, model => explorer.addModel("Main", model))
			}
		}

		const b = popover.$find(".btr-explorer-parent")
		b.replaceWith(explorer.element)
explorer.element.style.zIndex = "999";
explorer.element.style.marginTop = "40px";
explorer.element.style.transform = "translateX(45%)";
...
pageInit.itemdetails = function(category, assetId) {
	...
	if(settings.general.robuxToUSD) {
		...
		modifyTemplate("recommendations", template => {
			...
			if(label) {
				...
				if(IS_DEV_MODE) {
					alert("BTPekora modifyTemplate('recommendations'): Missing label")
				}
			}
		})
	}
	...
	document.$watch("#AjaxCommentsContainer").$then().$watch(".comments").$then()
		.$watchAll(".comment-item", comment => {
			...
			const fixedDate = PekoraTime(span.textContent.replace("|", ""))
			if(fixedDate) {
	...
	if(settings.itemdetails.addOwnersList) {
		...
		const setupOwnersList = (parent, name, ownerAssetId = assetId) => {
			...
			const createElement = ({ userId, userName, item }) => {
				...
				const thumbUrl = userId
					? `https://www.pekora.zip/headshot-thumbnail/image?userId=${userId}&width=60&height=60&format=png`
			...
			const getNames = request => {
				...
				return $.fetch(`https://users.pekora.zip/v1/users`, {
					method: "POST",
					headers: {
			...
			const loadOwners = () => {
				...
				const url = `https://inventory.pekora.zip/v2/assets/${ownerAssetId}/owners?limit=100&cursor=${cursor}`
				const maxRetries = 10
		...
		if(category === "bundles") {
			document.$watch(
				...
				firstItem => {
					const itemId = firstItem.href.replace(/^.*Pekora\.com\/[^/]+\/(\d+).*$/, "$1")
	...
	if(settings.itemdetails.showSales) {
		...
		if(category === "game-pass") {
			$.fetch(`https://api.pekora.zip/marketplace/game-pass-product-info?gamePassId=${assetId}`).then(async resp => {
				if(!resp.ok) { return }
				apply((await resp.json()).Sales)
			})
		} else if(category === "bundles") {
			const url = "https://catalog.pekora.zip/v1/catalog/items/details"
	...
	document.$watch(".container-0-2-41", itemCont => {
		//console.log("what", itemCont)
		itemCont.$watch('[class^="attrVal"]', el => {
			const assetTypeName = el.textContent.split(" | ")[1];
			//console.log("duhh", assetTypeName);
			//console.log("wtf", assetTypeName)
			...
			//console.log("huh", canAccessPromise)
			canAccessPromise.then(canAccess => {
				...
				if(settings.itemdetails.downloadButton && !InvalidDownloadableAssetTypeIds.includes(assetTypeId) && !strictDisabled) {
					...
					const createDownloadButton = actualUrl => {
						const btn = html`<a class="btr-download-button"><div class="btr-icon-download" style="margin-right: 10px;margin-top: -21px;"></div></a>`
					
							document.$watch('.section-content.noShadow[class*="itemContainer"]', p => {
		//console.log("1found noshadow!")
	
		p.$watch('[class*="itemDetailsContainer"]', c => {
			//console.log("1found item details container!")
			c.$watch('[class*="itemHeaderContainer"]', g => { 
				//console.log("1found item header!")
				g.$watch('[class*="dropdownContainer"]', j => {
					j.children[0].style.order = 1;
					//console.log("1found dropdown!")
					j.append(btn)
					g.classList.add("btr-download-btn-shown")
				})
			})
		})
	})
			...
			if(settings.itemdetails.whiteDecalThumbnailFix && assetTypeId === 13) {
				...
				const fixThumb = () => {
					...
					AssetCache.loadModel(assetId, model => {
						...
						newThumb = `https://assetgame.pekora.zip/asset-thumbnail/image?assetId=${imgId}&width=420&height=420`
