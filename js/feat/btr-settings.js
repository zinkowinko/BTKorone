...
const SettingsDiv = (() => {
	...
	<div class=btr-settings-modal style=opacity:0>
		<div class=btr-settings>
			<div class=btr-settings-header>
				<div class=btr-settings-header-title>BTKorone</div>
			...
			<div class="btr-settings-content selected" id=btr-settings-main data-name=main>
				<group label=General path=general>
					<div>
						...
						<checkbox label="Legacy Theme Compatibility Mode" path=disablePekoraThemes></checkbox>
					</div>
					...
					<div style=margin-top:5px>
						<checkbox label="Show Robux to BC Stipend" path=robuxToUSD></checkbox>
						<select path=robuxToUSDRate>
							<option selected disabled>R$ to BC Rate: (%opt%)</option>
							<optgroup label="Tiers">
								<option value=obc85>OBC (85 Daily)</option>
								<option value=tbc35>TBC (35 Daily)</option>
								<option value=bc25>BC (15 Daily)</option>
							</optgroup>
						</select>
					</div>
				</group>
				...
				<group label=Groups path=groups toggleable>
					<checkbox label="Show Rank in Group Wall" path=rankInWall></checkbox>
					<checkbox label="Paged Group Wall" path=pagedGroupWall></checkbox>
					<checkbox label="Small Social Links" path=modifySmallSocialLinksTitle></checkbox>
				</group>
				<group label="Game Details" path=gamedetails toggleable>
					...
					<checkbox label="Mock VIP Servers" path=mockVIPServers></checkbox>
				</group>
	...
	const initShoutFilters = () => {
		...
		loggedInUserPromise.then(async userId => {
			const resp = await $.fetch(`https://api.pekora.zip/users/${userId}/groups`)
			...
			json.sort((a, b) => (a.Name < b.Name ? -1 : 1)).forEach(group => {
				...
				<li class=btr-filter-group title="${group.Name}" draggable=true>
					<div class=btr-filter-group-icon>
						<img src="https://assetgame.pekora.zip/asset-thumbnail/image?assetId=${group.EmblemId}&width=150&height=150&format=png" draggable=false>
					</div>
