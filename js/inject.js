...
const INJECT_SCRIPT = () => {
	...
	function PreInit() {
		...
		if(window.googletag) {
			if(IS_DEV_MODE) {
				console.warn("[BTKorone] Failed to load inject before googletag")
			}
		} else {
	...
	function DocumentReady() {
					...
					const createCustomPager = ({ $scope }) => {
						...
						const loadMorePosts = () => {
							...
							return loadMorePromise = new Promise(async resolve => {
								...
								const baseUrl = `https://groups.pekora.zip/v2/groups/${groupId}/wall/posts?sortOrder=Desc&limit=100&cursor=`
		...
		if(settings.general.fixAudioPreview) {
			...
			ContentJS.listen("fixAudioPreview", (url, blobUrl) => {
				...
				console.warn("[BTKorone] Fixed broken audio previewer")
		...
		if(typeof Pekora !== "undefined") {
			if(settings.general.hideAds) {
				if(Pekora.PrerollPlayer) {
					Pekora.PrerollPlayer.waitForPreroll = x => $.Deferred().resolve(x)
				}

				if(Pekora.VideoPreRollDFP) {
					Pekora.VideoPreRollDFP = null
				}
			}
			...
			if(currentPage === "gamedetails" && settings.gamedetails.enabled) {
				...
				const createPager = gameInstance => {
					...
					$(".rbx-running-games-footer > .pager").hide() // Hide Pekora+ pager?
				...
				const init = () => {
					if(settings.gamedetails.addServerPager) {
						createPager(Pekora.RunningGameInstances)
					}
				...
				if(Pekora.RunningGameInstances) {
					setTimeout(init, 0)
				}
			} else if(currentPage === "develop") {
				if(Pekora.BuildPage) {
					Pekora.BuildPage.GameShowcase = new Proxy(Pekora.BuildPage.GameShowcase || {}, {
			...
			if(IS_DEV_MODE) {
				alert("[BTR] window.Pekora not set")
			}
		}
	...
	ContentJS.listen("linkify", cl => {
		...
		if(window.Pekora && Pekora.Linkify) { target.linkify() }
