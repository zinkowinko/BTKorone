...

{
	...
	const clearNotif = (notifId, didClick) => {
		...
		if(didClick) {
			chrome.tabs.create({ url: `https://www.pekora.zip/groups/${groupId}/Redirect` })
	...
	const loadMyShouts = async () => {
		const userId = await fetch("https://www.pekora.zip/game/GetCurrentUser.ashx", { credentials: "include" }).then(resp => (resp.ok ? resp.text() : null))
		if(!Number.isSafeInteger(+userId)) { return }

		const json = await fetch(`https://www.pekora.zip/apisite/groups/v1/users/${userId}/groups/roles`).then(resp => (resp.ok ? resp.json() : null))
		if(!json) { return }
	...
	const executeCheck = async () => {
		...
		if(!thumbsToGet.length) {
			...
			const url = `https://www.pekora.zip/apisite/thumbnails/v1/groups/icons?groupIds=${thumbsToGet.map(x => x.groupId).join(",")}&size=150x150&format=png`
