...
const OwnerAssetCache = {
	...
	async getLoggedInUser() {
		...
		try {
			const resp = await fetch(`https://www.pekora.zip/game/GetCurrentUser.ashx`, { credentials: "include" })
	...
	async request(next, populate = false, cursor = "") {
		...
		const url = next.type === "bundles"
			? `https://catalog.pekora.zip/v1/users/${this.data.lastUserId}/bundles?sortOrder=Desc&limit=${populate ? 100 : 10}${cursorParam}`
			: `https://inventory.pekora.zip/v2/users/${this.data.lastUserId}/inventory/${next.type}?sortOrder=Desc&limit=${populate ? 100 : 10}${cursorParam}`
