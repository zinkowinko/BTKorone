	...
	class Explorer {
		constructor() {
			...
			<div class="btr-explorer-parent">
				...
				<div class="btr-properties" style="left: -100%;">
				...
				group.Properties.sort(sortProperties).forEach(([name, prop]) => {
					...
					case "string": {
						...
						if(tooLong || value.includes("\n")) {
							...
							if(id) {
								const more = html`<a class=more href="https://pekora.zip/library/${id}/Redirect" target=_blank>🔗</a>`
								more.title = "View Asset"
								valueItem.append(more)
							}
		...
		addModel(title, model) {
			...
			const btn = html`<div class="btr-explorer-view-btn selected" style="min-width: 100%;text-align: center;">${title}</div>`
