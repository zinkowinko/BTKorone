...
const RBXPreview = (() => {
	...
	function getAvatarRules() {
		if(!avatarRulePromise) {
			avatarRulePromise = tryGet(`https://avatar.pekora.zip/v1/avatar-rules`)
		}
	...
	function getOutfitData(id) {
		if(!outfitCache[id]) {
			const outfitPromise = tryGet(`https://avatar.pekora.zip/v1/outfits/${id}/details`)
	...
	function getPlayerAppearance(userId) {
		if(!outfitCache["user" + userId]) {
			const outfitPromise = tryGet(`https://avatar.pekora.zip/v1/users/${userId}/avatar`)
	...
	function getDefaultAppearance() {
		if(!outfitCache.default) {
			const outfitPromise = tryGet(`https://avatar.pekora.zip/v1/avatar`)
...
class ItemPreviewer extends RBXPreview.AvatarPreviewer {
				...
				<div class=btr-body-outfits>
					...
					<div class="btr-body-outfit-btn selected" data-outfit=current>
						<div class=btr-body-outfit-icon>
							<img src="https://www.pekora.zip/avatar-thumbnail/image?userId=4719353&width=150&height=150&format=png">
						</div>
					...
					<div class=btr-body-outfit-btn data-outfit=default>
						<div class=btr-body-outfit-icon>
							<img src="https://www.pekora.zip/outfit-thumbnail/image?width=150&height=150&format=png&userOutfitId=1116516198">
						</div>
	...
	setBundleOutfit(outfitId) {
		...
		if(outfitId) {
			...
			this.bundleOutfitBtn.$find("img").src = `https://www.pekora.zip/outfit-thumbnail/image?width=150&height=150&format=png&userOutfitId=${outfitId}`
		} else {
