...
const AssetCache = (() => {
	...
	function resolveAssetUrlParams(url) {
		if(url.startsWith("rbxassetid://")) {
			url = `https://assetdelivery.pekora.zip/v1/asset/?id=${url.slice(13)}`
		} else if(url.startsWith("rbxhttp://")) {
			url = `https://www.pekora.zip/${url.slice(10)}`
		}
	...
	function resolveAssetUrl(url, finished) {
		...
		finished(resolveCache[paramString] = `https://assetdelivery.pekora.zip/v1/asset/?${paramString.toString()}`)
	}
	...
	return {
		...
		toAssetUrl(id) {
			return `https://assetdelivery.pekora.zip/v1/asset/?id=${+id}`
		},
